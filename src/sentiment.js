...
async function analyzeMentions(poolText) {
  ...
  const prompt =
`אתה אנליסט מדיה ותקשורת. לפניך אזכורים והודעות מהשיח הציבורי (קבוצות/ערוצים/חדשות). נתח אותם ביחס לח"כ *אריאל קלנר* (הליכוד).

החזר JSON בלבד, במבנה המדויק הזה:
{
  "kellner_mentions": <מספר ההודעות שמזכירות/עוסקות בקלנר>,
  ...
  "risks": [ "<נרטיב שלילי/מתקפה שדורשים תגובה — שורה קצרה>" ]
}

כללים:
- סַווג רק על סמך התוכן. אל תמציא. אם אין אזכורים של קלנר — kellner_mentions=0 והשאר ריק/אפס.
- ב-share_of_voice ספור כמה הודעות מזכירות כל דמות מהרשימה: אריאל קלנר, ${rivalLine}.
- narratives: עד 6, מהבולט לשקט.
- עברית בלבד בערכי הטקסט.
